...
当 converge 自身（SKILL.md、refs/、scripts/）完成自举收敛后，其复杂度满足设计审查的触发条件，但不应由 converge 自身的 Orchestrator 触发——这形成"Orchestrator 审查自己刚收敛的框架"的自指循环。自举后的设计审查由用户手动触发，或者由外部 Reviewer（独立会话、不同角色）执行。
