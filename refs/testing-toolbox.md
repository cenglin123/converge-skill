...
**执行方式**：
1. Orchestrator 在注入 `<test_command>` 时，若所有检测步骤（§二 1-4）均未命中，但审查对象包含可执行脚本，则注入 `null` 但附带 `cli_verify: true` 标记
2. Reviewer 收到 `cli_verify: true` 后，自行构造最小 happy-path（如 `init → dispatch → complete → dispatch → done`），Running 实际 CLI 命令验证基本行为是否符合文档描述
3. 若文档中描述了预期的 CLI 行为但 Reviewer 无法在环境中复现（如依赖缺失），标注 `cli_verify: skipped` 并说明原因
