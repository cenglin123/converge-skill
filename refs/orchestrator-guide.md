...
**案例摘要（self-contained）**：某 fork-executor pilot 设计复评经历 3 轮：每轮阻断均合理深挖（执行细节 → 模块边界 → 元问题「是否值得做」），修上一轮阻断竟引发下一轮此前不可讨论的新议题（rigor-escalation loop），验证 apparatus 复杂度反超被验证特性，可接受距离横移不缩。宪法第一部仲裁（Bitter Lesson 判「重读成本随上下文经济学改善而收缩，fork-executor 为随模型进步贬值的补丁」+ Occam 判「为贬值补丁投未收敛的重型验证比例失衡」）裁决：当轮修补无法消解元重设诉求，该 pilot 实为「收敛一个错误的问题」→ 放弃。完整 3 轮复评经过见 `GOVERNANCE-DECISIONS.md` GD-2（可选扩展阅读，非必要依赖）。
...
> **placement 伴随说明**：本节内容原停于 `GOVERNANCE-DECISIONS.md` GD-2 § 判例（audit 层）。但判据处置原则要求 Orchestrator 能察觉发散并层升至用户——audit 层 Orchestrator 运行时不读，判据放 audit 层 = 功能为零（可达性悖论）。迁移至操作指导层（Orchestrator 必读集）dissolve 此悖论。GD-2 § 判例 保留作历史快照（append-only 不回改），本节为唯一 live source。层属推理工具（四层 placement 判据）是本次迁移专用 N=1 工具，不进 CONSTITUTION、不覆盖 refs/ 其余文件。
